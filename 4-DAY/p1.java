...
class Board {
    ...
    private List<Integer> get_winning_row_or_column(Position pos) {
        List<Integer> winning_row_or_column;
        if(rows_match.get(pos.getX()) == 5)
            winning_row_or_column = coordinates.entrySet().stream().filter(set -> set.getValue().getX() == pos.getX()).map(set -> set.getKey()).collect(Collectors.toList());
        else
            winning_row_or_column = coordinates.entrySet().stream().filter(set -> set.getValue().getY() == pos.getY()).map(set -> set.getKey()).collect(Collectors.toList());
        return winning_row_or_column;
    }
